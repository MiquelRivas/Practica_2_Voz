...
data <- data %>% 
  dplyr::mutate(higiene_vocal = case_when(
    higiene_vocal == "Bec molta d'aigua" ~ "Bebe mucha agua",
    higiene_vocal == "Bec molta d'aigua, No seguesc cap rutina per a la veu" ~ "Bebe mucha agua",
    higiene_vocal == "Escalf la veu abans de començar la feina, Faig repòs vocal després de la feina" ~ "Calienta y hace reposo",
    higiene_vocal == "Escalf la veu abans de començar la feina, Faig repòs vocal després de la feina, Bec molta d'aigua" ~ "Calienta, hace reposo y bebe mucha agua",
    higiene_vocal == "Faig repòs vocal després de la feina" ~ "Hace reposo",
    higiene_vocal == "Faig repòs vocal després de la feina, Bec molta d'aigua" ~ "Hace reposo y bebe mucha agua",
    higiene_vocal == "Faig repòs vocal després de la feina, Bec molta d'aigua, No seguesc cap rutina per a la veu" ~ "Hace reposo y bebe mucha agua",
...
```
Para facilitarnos el trabajo a la hora de hacer el test de inferencia, que en este caso será de correlación, convertiremos las respuestas en valores numéricos:
```{r}
datos <- data %>%
...
print(resultado_cor)
```
Como podemos ver, al ser el resultado del test una correlación del 0.3, pese a ser relativamente moderada, se puede afirmar que las rutinas de higiene vocal tienen correlación con el VHI. Por tanto, se puede afirmar que el cuidado de la voz influye en la calidad de esta.
